...
template <typename T> class formula;
...
public:
    explicit term(T value) : m_parents({}), m_on_change({}), m_value(value) { }
...
auto operator+(term<T>& lhs, term<T>& rhs) -> formula<T> {
    auto add = [](const auto& lhss, const auto& rhss) -> T { return lhss + rhss; };
...
public:
    explicit formula(unary_expr<T> expr) : m_needs_update(true), m_expr(expr), m_cached_val(eval()) { }
    explicit formula(bin_expr<T> expr) : m_needs_update(true), m_expr(expr), m_cached_val(eval()) { }

    auto set_needs_update() -> void {
        m_needs_update = true;
    }

    auto eval() -> T {
        if (!m_needs_update) { return m_cached_val; }
        return std::visit(overloaded {
            [](unary_expr<T> expression) -> T {
                auto inner = std::visit(overloaded {
                    [](term<T>* val) -> T { return val->unwrap(); },
                    [](formula<T>* ast) -> T { return (ast->m_needs_update ? ast->eval() : ast->m_cached_val); }
                }, expression.rhs);
                return expression.op(inner);
            },
            [](bin_expr<T> operand) -> T {
                auto inner_lhs = std::visit(overloaded {
                    [](term<T>* val) -> T { return val->unwrap(); },
                    [](formula<T>* ast) -> T { return ast->eval(); }
                }, operand.lhs);
                auto inner_rhs = std::visit(overloaded {
                    [](term<T>* val) -> T { return val->unwrap(); },
                    [](formula<T>* ast) -> T { return ast->eval(); }
                }, operand.rhs);
